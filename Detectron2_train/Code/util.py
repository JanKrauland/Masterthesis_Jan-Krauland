...
def get_cfg(output_dir, batch_size, iterations, checkpoint_period, model, device, nmr_classes):
    """
...
def get_dicts(json_path):#img_dir, ann_dir):
    """
    ...
    for image_info in annotation_data:
        record = {}
        record["file_name"] = image_info["file_name"]#.replace("G:\\Meine Ablage\\", "/content/drive/MyDrive/").replace("\\", "/")
...
def register_datasets(root_dir, class_list_file):
    ...
    DatasetCatalog.register('train', lambda: get_dicts(os.path.join(root_dir, 'train_annotations.json')))
    MetadataCatalog.get('train').set(thing_classes=classes_)
    DatasetCatalog.register('val', lambda: get_dicts(os.path.join(root_dir, 'test_annotations.json')))
    MetadataCatalog.get('val').set(thing_classes=classes_)       
    DatasetCatalog.register('test', lambda: get_dicts(os.path.join(root_dir, 'test_annotations.json')))
    MetadataCatalog.get('test').set(thing_classes=classes_)
    return len(classes_)

def train(output_dir, data_dir, class_list_file, batch_size, iterations, checkpoint_period, device,
          model):
    """
    ...
    cfg = get_cfg(output_dir, batch_size, iterations, checkpoint_period, model, device, nmr_classes)

    # Create the output directory
    os.makedirs(cfg.OUTPUT_DIR, exist_ok=True)
    # Create the trainer object
    trainer = CocoTrainer(cfg) #DefaultTrainer(cfg)
